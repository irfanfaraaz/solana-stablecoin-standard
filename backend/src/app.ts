...
app.use((req: Request, _res, next) => {
  ...
  (req as Request & { log: ReturnType<typeof createLogger> }).log =
    createLogger(req.requestId);
  next();
});
...
app.post("/screen", async (req: Request, res: Response) => {
  const logger = (req as Request & { log: ReturnType<typeof createLogger> })
    .log;
  try {
    const { address: addressStr, mint: mintStr } = req.body as {
      address: string;
      mint: string;
    };
    if (!addressStr || !mintStr)
      return res.status(400).json({ error: "address and mint required" });
...
app.get("/screen", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!addressStr || !mintStr)
      return res
        .status(400)
        .json({ error: "query params address and mint required" });
    const mint = new PublicKey(mintStr);
    const address = new PublicKey(addressStr);
    const sdk = getSdk(mint);
    const result = await screenAddress(sdk, address);
...
app.post("/mint", async (req: Request, res: Response) => {
  ...
  try {
    const {
      ...
      recipient,
      amount,
    } = req.body as { mint?: string; recipient: string; amount: string };
    ...
    if (!screening.allowed)
      return res
        .status(403)
        .json({ error: screening.reason || "Screening failed" });
...
app.post("/burn", async (req: Request, res: Response) => {
  ...
  try {
    const {
      ...
      amount,
      from: fromStr,
    } = req.body as { mint?: string; amount: string; from?: string };
    const mint = new PublicKey(mintStr || DEFAULT_MINT);
    const keypair = loadKeypair();
...
app.post("/blacklist/add", async (req: Request, res: Response) => {
  ...
  try {
    const {
      ...
      address: addressStr,
      reason,
    } = req.body as { mint?: string; address: string; reason?: string };
    const mint = new PublicKey(mintStr || DEFAULT_MINT);
    const address = new PublicKey(addressStr);
    const keypair = loadKeypair();
    const sdk = getSdk(mint);
...
app.post("/blacklist/remove", async (req: Request, res: Response) => {
  const logger = (req as Request & { log: ReturnType<typeof createLogger> })
    .log;
  try {
    const { mint: mintStr, address: addressStr } = req.body as {
      mint?: string;
      address: string;
    };
    const mint = new PublicKey(mintStr || DEFAULT_MINT);
    ...
    const sig = await tx.rpc();
    logAudit("blacklist_remove", {
      mint: mint.toBase58(),
      address: addressStr,
      signature: sig,
    });
    const slot = await getSlotForSignature(sig);
    dispatchWebhook("blacklist_remove", {
...
app.get("/audit/export", (req: Request, res: Response) => {
  ...
  if (format === "csv") {
    const header =
      "time,event,mint,signature,recipient,amount,from,address,reason,treasury\n";
    const esc = (v: unknown) =>
