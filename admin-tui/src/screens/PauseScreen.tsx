...
const ITEMS: Item[] = [
  { label: "Yes, pause mint", value: "confirm" },
...
export function PauseScreen() {
  ...
  const handleSelect = async (item: Item) => {
    ...
    try {
      await runTx(
        { pendingMessage: "Pausing…", successMessage: "Mint paused" },
        () => service.pause(state.programs!.wallet.publicKey)
      );
    } catch {
      // Error set by runTx
    }
  ...
  return (
    <Box flexDirection="column">
      <SectionHeader title="PAUSE" />
      <Text dimColor>Disables all transfers, mint, burn</Text>
      <Box marginTop={1}>
