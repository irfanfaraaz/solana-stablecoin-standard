 ...
 */
export async function builderToInstruction(builder: unknown): Promise<TransactionInstruction> {
  const b = builder != null && typeof (builder as Promise<unknown>).then === "function"
  ...
  if (typeof anyB?.prepare === "function") {
    const out = await (anyB.prepare as () => Promise<{ instruction?: TransactionInstruction }>)();
  ...
  if (anyB?.instruction != null && typeof (anyB.instruction as Promise<TransactionInstruction>).then === "function") {
    return await (anyB.instruction as Promise<TransactionInstruction>);
  }
  ...
  const proto = b != null ? Object.getPrototypeOf(b) as Record<string, unknown> : null;
  if (proto && typeof proto.instruction === "function") {
    return await (proto.instruction as () => Promise<TransactionInstruction>).call(b);
  }
  if (proto && typeof proto.prepare === "function") {
    const out = await (proto.prepare as () => Promise<{ instruction?: TransactionInstruction }>).call(b);
    if (out?.instruction) return out.instruction;
  }

  if (typeof anyB?.transaction === "function") {
    const tx = await (anyB.transaction as () => Promise<{ instructions?: TransactionInstruction[] }>)();
