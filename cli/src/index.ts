...
function parseTomlConfig(configPath: string): Partial<StablecoinConfig> & {
  name: string;
  symbol: string;
  decimals: number;
  uri?: string;
...
program
  ...
  .requiredOption(
    "-n, --name <name>",
    "Token name (required when not using --custom)"
  )
  ...
  .action(async (opts) => {
    ...
    const { stablecoinProgram, transferHookProgram } = loadPrograms(
      connection,
      wallet
    );
    ...
    if (opts.custom) {
      ...
      config = {
        ...
        enablePermanentDelegate: !!custom.enablePermanentDelegate,
        enableTransferHook: !!custom.enableTransferHook,
        defaultAccountFrozen: !!custom.defaultAccountFrozen,
      };
    } else {
      const preset =
        opts.preset === "sss-1"
    ...
    if (config.enableTransferHook && transferHookProgram) {
      ...
      const hookTx = await compliance.initializeTransferHookExtraAccounts(
        keypair.publicKey,
      ...
      const minterSig = await minterTx.rpc();
      output(
        {
          mint: mint.toBase58(),
          configPda: SolanaStablecoin.getConfigPDA(
            mint,
            stablecoinProgram.programId
          ).toBase58(),
          signature: sig,
          transferHookInitSignature: hookSig,
          minterInitSignature: minterSig,
        },
...
function isAccountNotInitializedError(err: unknown): boolean {
  const msg = err instanceof Error ? err.message : String(err);
  return (
    /account not initialized/i.test(msg) ||
    /AccountNotInitialized/i.test(msg) ||
    /uninitialized/i.test(msg) ||
    /custom program error:\s*0x9\b/i.test(msg) ||
    msg.includes("0x9")
  );
}

function isMinterInactiveError(err: unknown): boolean {
  const msg = err instanceof Error ? err.message : String(err);
  return (
    /MinterInactive/i.test(msg) ||
    /minter is inactive/i.test(msg) ||
...
program
  ...
  .requiredOption(
    "-m, --mint <address>",
    "Mint address (or use symbol and pass mint PDA)"
  )
  .action(async (recipient, amount, opts) => {
    ...
    const doMint = async (): Promise<string> => {
      ...
      const mintBuilder = await sdk.mint(
        keypair.publicKey,
        recipientPubkey,
        amount
      );
      const mintIx = await (
        mintBuilder as { instruction(): Promise<web3.TransactionInstruction> }
      ).instruction();
      instructions.push(mintIx);
      const tx = new Transaction().add(...instructions);
      const provider = stablecoinProgram.provider as AnchorProvider;
      return await provider.sendAndConfirm(tx, [keypair]);
    };

    try {
      const sig = await doMint();
      output({ signature: sig }, jsonOut);
    } catch (err) {
      if (isMinterInactiveError(err)) {
        const hint = `Minter is inactive. Add yourself as minter with: yarn cli minters add ${keypair.publicKey.toBase58()} -m ${mint.toBase58()}`;
        if (jsonOut) output({ error: hint }, true);
      ...
      if (isAccountNotInitializedError(err)) {
        if (!jsonOut)
          console.error("Recipient token account not found. Creating it...");
        try {
          const sig = await doMint();
          output({ signature: sig }, jsonOut);
        } catch (retryErr) {
...
const rolesCmd = program
  .command("roles")
  .description(
    "View or update role accounts (burner, pauser, blacklister, seizer)"
  )
  .requiredOption("-m, --mint <address>", "Mint address");
...
rolesCmd
  .command("update")
  .description(
    "Update one or more roles (master authority only). Omitted roles are unchanged. To revoke, set role to master pubkey."
  )
  ...
  .action(
    async (
      ...
      if (opts.blacklister != null)
        roles.blacklister = new PublicKey(opts.blacklister);
      if (opts.seizer != null) roles.seizer = new PublicKey(opts.seizer);
      if (Object.keys(roles).length === 0) {
        output(
          {
            error:
              "Provide at least one role: --burner, --pauser, --blacklister, or --seizer",
          },
          (program.opts() as any).json
        );
        process.exitCode = 1;
      ...
      const sig = await tx.rpc();
      output(
        { signature: sig, updated: Object.keys(roles) },
        (program.opts() as any).json
      );
    }
  );
...
program
  ...
  .action(async (opts) => {
    ...
    const supply = await sdk.getTotalSupply();
    output(
      { mint: mint.toBase58(), supply: supply.toString() },
      (program.opts() as any).json
    );
  });
...
program
  ...
  .addCommand(
    new Command("add")
      ...
      .action(async (address, opts, parent) => {
        const keypair = loadKeypair((program.opts() as any).keypair);
        const connection = getConnection((program.opts() as any).rpcUrl);
        const wallet = new Wallet(keypair);
        const { stablecoinProgram, transferHookProgram } = loadPrograms(
  ...
  .addCommand(
    new Command("remove")
      ...
      .action(async (address, opts, parent) => {
        ...
        const compliance = new SSSComplianceModule(sdk);
        const tx = await compliance.removeFromBlacklist(
          keypair.publicKey,
          new PublicKey(address)
        );
        const sig = await tx.rpc();
...
program
  ...
  .addCommand(
    new Command("add")
      .argument("<wallet>")
      .action(async (walletAddress, _opts, parent) => {
        const keypair = loadKeypair((program.opts() as any).keypair);
        const connection = getConnection((program.opts() as any).rpcUrl);
        const wallet = new Wallet(keypair);
        const { stablecoinProgram, transferHookProgram } = loadPrograms(
...
program
  ...
  .action(async (from: string, opts: any) => {
    ...
    const amount =
      opts.amount === "0"
        ? (await sdk.getTotalSupply()).toString()
        : opts.amount;
    const tx = await compliance.seize(
      keypair.publicKey,
      new PublicKey(from),
...
mintersCmd
  .command("list")
  .description("List minters (info)")
  .action(async (_opts: any, cmd?: Command) => {
    const mint =
      (cmd?.parent as Command)?.opts?.()?.mint ??
      (mintersCmd as any).opts?.()?.mint;
    output(
      { message: "Use status for config/roles; minters are per-PDA", mint },
      (program.opts() as any).json
    );
  });
mintersCmd
  .command("add <minter_pubkey>")
  .description("Add a minter with quota")
  .option("-q, --quota <quota>", "Daily mint quota", "1000000000")
  .action(async (minterPubkey: string, opts: any, cmd?: Command) => {
...
mintersCmd
  ...
  .description("Remove / deactivate a minter")
  .action(async (minterPubkey: string, opts: any, cmd?: Command) => {
    const parent = cmd?.parent as Command | undefined;
    const mint = parent?.opts?.()?.mint ?? (mintersCmd as any).opts?.()?.mint;
    const keypair = loadKeypair((program.opts() as any).keypair);
    ...
    const sig = await tx.rpc();
    output(
      { signature: sig, minter: minterPubkey },
      (program.opts() as any).json
    );
  });
...
program
  ...
  .action(async (opts) => {
    ...
    output(
      { mint: opts.mint, holders, count: holders.length },
      (program.opts() as any).json
    );
  });
...
program
  ...
  .option(
    "--format <format>",
    "Export format when using backend: json or csv",
    "json"
  )
  .option(
    "--action <type>",
    "Filter by action type (mint, burn, blacklist_add, etc.)"
  )
  .action(
    async (opts: { backendUrl?: string; format?: string; action?: string }) => {
      const jsonOut = (program.opts() as any).json;
      if (opts.backendUrl) {
          ...
          if (format === "csv") {
            ...
            if (opts.action) {
              ...
              events = events.filter(
                (e: unknown) =>
                  ((e as { event?: string }).event ?? "").toLowerCase() ===
                  action
              );
            }
        ...
        output(
          {
            message:
              "Provide --backend-url to fetch audit log from the backend API (e.g. --backend-url http://localhost:3000). See docs/API.md and backend README.",
          },
          jsonOut
        );
      }
